...
class AddAppointment extends React.PureComponent {
  state = {
    patient: this.props.getPatients()[0].id,
    procedure: this.props.getProcedures()[0].id,
  }
    ...
    return(
      <React.Fragment>
        <Grid container direction="column" alignItems="center">
          ...
          <Select value={this.state.procedure} style={{width: '80%'}} onChange={(e) => { console.log(e); this.setState({ procedure: e.target.value }) }}>
...
const mapDispatchToProps = dispatch => ({
  addAppointment: (appointment) => {dispatch(appointmentActions.addAppointment(appointment))}
});
